...
- ⚠️ **La révélation NOMME la famille en toutes lettres** (« carte », « avatar », « variante » — `KIND_LABELS`, le même mot que le barème), elle ne la dit pas en emoji : 🃏 / 🎭 / 🎨 ne se distinguaient pas, et c'est la famille qui décide où l'objet se retrouve (Profil pour un avatar, DeckBuilder pour une variante). Ce qui reste une surprise, c'est ce qui ATTEND le tap (`🎁 ×N`), pas ce qui vient d'être tiré.
...
- **L'annonce porte un FOND** (`.round-intro-veil`), et ce n'est pas un voile de `Modal` : un halo dégradé qui s'éteint sur les bords, donc le plateau reste visible partout et seul le centre s'assombrit. Sans lui, « TOUR 3 » tombait parfois sur une illustration de terrain claire. ⚠️ Il a sa **propre animation** : il doit être là avant le chiffre et s'attarder après lui — les faire partir ensemble donne un clignotement.
...
- ⚠️ Le registre ne porte que des **ids** — `logic/` n'importe pas `data/`. `data/DrawInfo.ts` (pur) met la pioche en mots ; `data/gameNames.bonusSourceName` résout les noms, pour ce registre **comme pour celui du multiplicateur** (cf. « Multiplicateur de dégâts »).
...
- `playerMultiplierSources` / `enemyMultiplierSources` sont le **registre de provenance**, jumeau de celui de la pioche : mêmes règles (`BonusSourceEntry`, des IDS seulement, versé EN MÊME TEMPS que le crédit) et même invariant, `sum(value) === le bonus`. Deux émetteurs : l'attribut (`AttributeManager` → `damage_multiplier_sources`, ce round) et la magie (`GameState.player_multiplier_sources`, permanent). Verrouillé par `end-round-multiplier.test.ts`.
- ⚠️ `data/gameNames.bonusSourceName` est le **seul** résolveur `(kind, ref) → nom` des deux registres, et `DrawInfo.BONUS_SOURCE_ICON` leur **seule** table de glyphes — la popup de pioche s'en était écrit une copie privée.
...
⚠️ **Une case désignée AVANT les matériaux est RETENUE, pas refusée** (`GameController._reserveCell`, champ `selectedCell`) : c'est ce qui rend une carte à matériaux glissable comme les autres — on lâche la carte sur sa case, on désigne ses matériaux, l'unité s'y pose. Le geste répondait « Sélectionne les matériaux d'abord » et ne retenait rien.
- ⚠️ **Elle vit dans `onCellTap`, pas dans le glisser** : le glisser-déposer appelle `onCellTap` en arrivant et n'écrit aucune règle. Loger la réservation dans le geste donnerait deux façons de désigner une case, dont une seule sait attendre.
- ⚠️ **Rien n'est validé à la réservation, et il n'y a rien à valider** : sans matériaux, `validCells` rend `[]` (la règle, pas une omission) et `canSummon` refuserait une case parfaitement légitime une fois la sélection faite — celle du matériau qu'on s'apprête à consommer, par exemple. Seule la **zone du joueur** se vérifie tout de suite. C'est `canSummon`, à la pose, qui tranche.
- ⚠️ **Trois réponses à « où », et leur ORDRE est la règle** : la **recette** d'abord (`GameSession.forcedCell`, simple ré-export de `InvocationManager.forcedCell` — une condition à un matériel impose la case de ce matériel, et rien ne passe devant), la case **retenue** ensuite, la case du **matériel** en dernier (le geste en un tap, inchangé).
- Une sélection achevée dont la case retenue ne convient plus la **libère en le disant** (la raison de `canSummon`) : le plateau s'allume alors de ses cases valides, qu'un repère violet ne doit pas contredire.
- ⚠️ **Le cimetière pose désormais, mais seulement sur une case retenue** : un matériau de cimetière n'a pas de case à offrir au résultat — c'est pourquoi `tapGraveyardUnit` n'a jamais posé de lui-même. Une invocation payée au seul cimetière est justement celle où il n'y a rien d'autre à taper ensuite.
- Repère à l'écran : `Scene3D.setSelectedPos`, celui du repositionnement — les deux disent « c'est ICI », et les deux états s'excluent. Verrouillé par `summon-drop-cell.test.ts`.
...
- **`represented_ids`** — les ids que l'unité « représente », **pré-déterminés sur la carte** (section « Lignée » de l'admin), jamais calculés à l'invocation. ⚠️ `Unit` y ajoute toujours son propre `card.id` : la donnée ne porte que la lignée **héritée**. Affiché au tooltip (🧬). ⚠️ **MULTISET, pas un ensemble** : le même id peut y figurer plusieurs fois (une composite issue de deux exemplaires de la même carte) — `Unit` ne le dédoublonne plus, et `materialLineageLegit` compare des **multiplicités**.
  - **Légitimité** (`materialLineageLegit`) : toute la lignée héritée d'un matériel doit être **elle-même exigée**, autant de fois qu'elle y figure, par la condition en cours. « Aile de feu » (Avian + Burstinatrix) ne remplace pas Avian seul, mais comble à elle seule les deux exigences d'une condition qui demande les deux — et une composite « Avian, Avian » ne remplace pas non plus un Avian seul : elle n'est légitime que pour une condition qui nomme Avian au moins deux fois.
...
- **`material_value`** — le nombre de slots que l'unité représente si elle est consommée **sur un slot libre** (nommée, elle n'en paie qu'un — cf. `materialSlotsPaid`). ⚠️ C'est une **donnée de carte**, saisie en admin, lue par le constructeur d'`Unit` pour les **deux camps** : elle était dérivée en quatre exemplaires dans le `switch`, si bien que l'IA et le joueur n'avaient pas la même règle. **Affichée** : pastille `◈N` au **haut-gauche** de la carte du plateau (`unit-mat-badge`), **la pastille de coût de `Card3D` à l'identique** (même coin, même boîte, même encre — c'est la même monnaie, là un prix ici une valeur, et le joueur fait l'aller-retour entre les deux bandes), **au-dessus de 1 seulement** — 1 est le défaut, une pastille partout ne distinguerait rien ; le tooltip d'une **unité** la dit toujours, c'est là qu'on vient chercher la réponse, et il précise « sur un slot libre » plutôt que de promettre une valeur qu'une recette qui la nomme ne rendra pas.

Un matériel `ARCH_*` désigne **n'importe quelle** unité portant l'attribut, pas une carte (`isAttributeMaterial`).

## L'échelle de ticks (`speed-scale.mjs`, racine)

Tout ce qui se chiffrait en ticks se paramètre sur **un compteur de 0 à 100, où plus haut veut toujours dire plus fort**. Une seule fenêtre (2 à 77 ticks), **deux lectures qui vont en sens inverse en ticks** :

| Lecture | Ce qu'elle chiffre | 0 | 100 |
|---|---|---|---|
| **Rythme** (`ticksForRate`) | attaque, déplacement, chargement de pouvoir | 77 ticks (lent) | 2 ticks (rapide) |
| **Durée** (`ticksForDuration`) | paralysie, blocage, confusion, provocation | 2 ticks (bref) | 77 ticks (long) |

```js
ticksForRate(rate)          // ceil(77 − 0,75 × C) · rateForTicks : l'inverse exact
ticksForDuration(d)         // ceil(2 + 0,75 × C) · durationForTicks : l'inverse exact
clampRate(v)                // LE clamp du compteur, pour ses deux lectures
rateDeltaForTickDelta(d) / RATE_STATS / DURATION_POWERS
```
...
- **Les données ne portent que le compteur** : `stats.attack_rate`, `stats.movement_rate`, `power.power_rate`, `power.duration`. Les ticks (`attack_period`, `movement_period`, `powerPeriod()`, les quatre décomptes de statut) sont **dérivés**, jamais saisis ni persistés.
- ⚠️ **Le sens de la durée est INVERSÉ par rapport au rythme, et il le faut** : les traduire dans le même sens ferait de « 100 » la paralysie la plus COURTE — le contresens exact que le compteur existe pour lever.
...
- ⚠️ **`power.duration` et `power.value` S'EXCLUENT** : les quatre pouvoirs de `DURATION_POWERS` lisent la durée et n'ont plus de `value` ; les dix autres l'inverse (`value` n'y a jamais chiffré des ticks). C'est cette séparation qui rend la reprise **idempotente** — `40` ne se distingue pas de `40`, `value` et `duration` toujours.
- ⚠️ **Une durée ABSENTE reste conforme**, contrairement à un compteur de vitesse : le moteur porte un repli par pouvoir (24 / 30 / 24 / 24, soit 20 / 25 / 20 / 20 ticks) et trois cartes livrées s'en servent. Corollaire : `powerDurationTicks` lit en `||` comme `powerValue`, donc **un compteur 0 vaut le repli**, pas deux ticks — le seul écart avec les rythmes, où 0 est légitime.
- ⚠️ **Seule la SÉVÉRITÉ de la paralysie reste en TICKS** (`attack_period_modifier`) : elle DOUBLE la période, et un doublement ne s'écrit pas comme un delta de compteur constant (−19 points sur une unité rapide, −51 sur une lente). Sa durée, elle, est un compteur comme les autres.
- ⚠️ **Une installation existante N'EST PAS reprise toute seule** : `bootstrap()` ne recopie jamais `initial-data/` sur un `data/` déjà peuplé, donc le volume garde son `attack_speed` en ticks. La panne est **muette et plausible** — `clampRate(undefined)` rend **0**, donc le rythme le plus LENT : les unités jouent au ralenti, l'admin affiche le milieu du curseur, le tooltip laisse deux cases blanches. Trois gardes la nomment désormais : `cardContract.missingRates` / `missingDurations` (**400** à l'écriture), `audit:cards --check` (sortie 1) et un avertissement `[catalogue]` au **démarrage du serveur**, qui nomme le remède.
- **Reprise de données** : `node scripts/migrate-speeds.js [--write] [--initial-data]`, idempotent, écriture atomique. Cible `data/` s'il existe, sinon `initial-data/` ; ⚠️ **`--initial-data` force la semence**, et les DEUX dossiers sont à reprendre — migrer l'un ne migre pas l'autre. Il **refuse** un `stat_modifier` multiplicateur sur un rythme (un multiplicateur de période n'a pas d'équivalent en compteur) plutôt que d'inventer.
- ⚠️ **Le catalogue n'occupe que le HAUT de l'échelle** : les vitesses d'attaque livrées tiennent entre 76 et 99, les déplacements entre 76 et 96 (visible tel quel dans l'onglet Stats de l'admin). Les durées, elles, s'étalent de 24 à 100. C'est la conséquence directe de la conversion littérale — le rééquilibrage qui répartira les cartes sur les 100 points reste **à faire**, à la main, en admin.
...
shield / power_gauge
power_id / power_rate / power_value / power_duration  // réécrits durablement par grant_power
                                                 // et power_cooldown ; ⚠️ power_duration EXCLUT power_value
...
- ⚠️ **La paralysie chiffre sa DURÉE, pas sa sévérité** (seul pouvoir dont le sens du champ a changé) : un `+6` plat coûtait trois quarts de ses attaques à une unité rapide et rien à une lente. Un doublement coûte **la moitié** quel que soit le rythme. Un second tir **rafraîchit** au lieu d'empiler.
...
- **Affichage** (`components/shopping/MagieCard.tsx`) : liseré gauche 4 px + chip texte, sur **les trois** paliers. Commune `--color-steel`, Rare `--color-azure`, Légendaire `--color-violet`. ⚠️ Les trois lisaient une couleur de **tier**, qui ne voulait rien dire ici et qui a changé sous elles le jour où les tiers ont repris la palette des cartes — la Légendaire y serait devenue **dorée**, exactement ce que la règle ci-dessous interdit. ⚠️ **Pas d'or pour la Légendaire** : le nom, le titre et le décompte sont déjà `text-gold` — il faut **contraster avec l'accent du panneau**. Vert et rouge portent déjà « validé » et « danger ».
...
- ⚠️ **PAS de badge de tier, et les deux badges sont à GAUCHE.** Le tier EST la couleur du cadre (`three/cardPalette`), ce que la vignette 2D ne savait pas faire : un « T3 » par-dessus un cadre violet dit deux fois la même chose et mange le seul coin qu'un éventail laisse voir. Le coin libéré prend le **coût** (`◈N`, haut-gauche — une pastille par recette de prix **distinct**, côte à côte, du moins cher au plus cher : `SummonInfo.summonCostsOf`), le compte d'exemplaires descend en **bas-gauche** — c'est la lisière GAUCHE qui reste visible sous la carte suivante, à droite le `×N` disparaissait. ⚠️ **La carte du PLATEAU suit les deux coins** : `unit-mat-badge` (ce qu'elle vaut) en haut, `unit-vet-badge` (ce qu'elle cumule) en bas. Le nom cède sa marge (`.card3d.has-count`) plutôt que de passer dessous.
- ⚠️ **Le nom et les badges sont dans la police de l'INTERFACE** (`--font-sans`), pas dans le serif d'affichage dont `.card3d` hérite de `.unit-card`. Le serif est juste sur le plateau, où la carte est un objet regardé de loin ; en main elle se LIT, et la vignette 2D qu'elle remplace posait le nom en sans. Deux polices pour le même nom d'une bande à l'autre se remarquaient.
- ⚠️ **Les deux rails du mode web sont TRANSPARENTS** : il ne reste que leur titre (« MAIN », « NEUTRALISÉES »). Ils portaient chacun un cadre teinté — sur un téléphone en paysage, deux pavés de couleur encadraient le plateau en permanence, et ce qu'il faut lire dans un rail est justement une COULEUR, celle du cadre des cartes. ⚠️ **Le portrait porte le même titre, et rien d'autre** (`ZONE_LABEL_PORTRAIT`, `hand/rail.ts`) : toujours pas de cadre ni de fond — un fond derrière une carte lui dispute la seule chose qu'elle ait à dire, sa couleur de cadre. Le titre est posé DANS la bande, au coin haut-gauche, et déclaré **avant** les cartes : à `z-index` égal l'ordre du DOM tranche, donc une main qui s'allonge recouvre son propre titre. Rien ne peut se poser au-dessus des bandes — main (`bottom-14`) et cimetière (`bottom-44`) ne laissent que 3 px entre eux.
- ⚠️ Les rails du mode web partent de **`top-16`** et non `top-28` : ils se lisaient posés trop bas en paysage. **64 px est le plancher, et il est MESURÉ** — la barre de PV descend exactement à 56 px, il reste 8 px de gouttière.
- ⚠️ **La pile d'un rail est alignée EN HAUT, pas centrée** (`railLayout`) : un rail est une colonne sous un titre, et deux cartes centrées dans 225 px flottaient à cinquante pixels de leur intitulé. C'est ce qui règle le cas d'une main courte, que remonter la bande ne pouvait pas régler. Corollaire : un débordement ne déborde plus que par le BAS, donc jamais sous le titre.
...
- ⚠️ **La PLACE est une donnée, le GESTE est une règle.** `cardFan` pose sa géométrie en **variables CSS** (`--card-x`, `--card-rot`, `--card-scale`, `--card-z`) et c'est `card3d.css` qui compose le `transform`. Un `transform` écrit en ligne par React aurait fermé la porte au survol : une déclaration en ligne bat toute règle de feuille, donc il aurait fallu un état React — un re-render de tout l'éventail à chaque carte survolée. Les variables du geste (`--card-lift-*`, `--card-straighten`, `--card-pop`) ne sont **jamais** posées en ligne, d'où l'absence d'`!important`.
- **Survol** : `@media (hover: hover) and (pointer: fine)` — la seule du projet. Un téléphone qui hérite d'un `:hover` le garde **collé** après le tap. La carte se lève, passe devant et se **redresse** ; son nom se donne alors en entier, sur deux lignes s'il le faut — c'est ce qui rend la troncature du repos acceptable. Sur mobile, le tap fait le même geste (la carte retenue).
- ⚠️ **Aucune boucle rAF** : transitions CSS seules. Même doctrine que le rendu à la demande de `Scene3D._animate`.
- ⚠️ **Ni `scale` au survol, ni `will-change: transform`** — les deux floutent le texte, et pour la même raison : le `rotateX` fait composer la carte en **couche**, rasterisée une fois puis transformée. Un `scale` en étire la texture ; `will-change` empêche de la re-rasteriser. Le nom sortait flou — celui-là même qu'on lève la carte pour lire. La mise en avant passe donc par la levée, le premier plan et le redressement, qui ne déforment rien. `will-change` promet des changements **fréquents** : un état qui change une fois n'en a pas besoin.
- ⚠️ **`prefers-reduced-motion` retire le MOUVEMENT, pas l'information** : la carte se lève toujours, se redresse toujours, donne toujours son nom en entier — elle y arrive d'un coup au lieu d'y glisser (`transition: none`, voile figé).
- **Aucune branche `LOW_END_DEVICE`, et c'est MESURÉ** : CPU bridé ×6, balayage de survol sur toute la main → médiane **16,7 ms**, p95 16,8 ms. La raison de fond est structurelle : **la main n'est jamais à l'écran pendant le combat**, donc les cartes ne partagent le GPU avec rien d'animé. Le jour où ça changerait, la mesure est à refaire avant d'ajouter quoi que ce soit.
- ⚠️ **Un glisser relâché APRÈS le tap sur PRÊT ne pose rien, et sans garde neuve** : `_clearSelection()` au moment de l'engagement vide `selectedCard`, et `onCellTap` ne fait rien sans elle. La capture du pointeur fait bien traverser le geste l'overlay d'attente PvP — c'est le dépôt qui devient un non-événement. **Ne pas ajouter de seconde garde** : la règle est déjà écrite à un endroit.
- **Glisser-déposer** (main seulement) : il n'écrit **aucune règle**. Il appelle `selectCard` en partant et `onCellTap` en arrivant — les deux points d'entrée du tap, donc les mêmes surlignages, la même **réservation de case** quand les matériaux manquent encore (cf. Invocation), le même menu de conditions multiples. Lâchée hors du plateau, la carte revient et la **sélection reste**. Absent en ciblage de magie (la carte y est une cible, pas une unité à poser) et au cimetière (un matériau se désigne, il ne se pose pas). Deux choses ajoutées côté `three/` : **`Scene3D.cellAtScreen(x, y)`** (accès : `GameController.cellAtScreen`) et **`Scene3D.setHoverCell(pos)`**, la case survolée (accès : `hoverCellAt` / `clearHoverCell`).
- ⚠️ **Le survol ne JUGE rien** : une case invalide n'est déjà pas allumée par `setHighlight`, et la validité ne se tranche qu'à la pose (`canSummon` refuserait une case légitime tant que les matériaux ne sont pas désignés). Il dit « ici », pas « ça passe ».
- ⚠️ `setHoverCell` **sort tôt quand la case ne change pas** : l'appelant est un `pointermove`, et `_refreshTileColors` repeint les 55 tuiles puis invalide le rendu — à chaque frame, il annulerait à lui seul le rendu à la demande de `_animate` pour toute la durée du geste.
- ⚠️ **La carte glissée monte de 80 % de sa hauteur au-dessus du doigt** (`.card3d.is-dragging`, `card3d.css`) : elle fait 84 px de large pour une case d'environ 72 et recouvrait donc exactement le surlignage qu'elle vient chercher. Rien ne change côté logique — le dépôt lit le POINT DU POINTEUR, jamais la boîte de la carte.
- ⚠️ **La PRISE EN MAIN tient dans UNE variable animable, `--card-raise`** (0 → 1, `@property <number>`), dont dérivent la montée, l'éloignement et le redressement : quatre transitions parallèles finiraient par se désaccorder. Elle est la seule ligne de `transition` du glisser — le `transform` n'y est pas, donc le SUIVI du doigt reste instantané (avec la transition du repos, la carte rattraperait le doigt 140 ms plus tard). Sans `@property`, la variable n'a pas de type donc rien à interpoler : la prise redevient un saut, jamais une carte cassée.
- ⚠️ **L'éloignement passe par `translateZ`, jamais par un `scale`** : la couche porte une `perspective`, donc la carte se rapproche réellement de l'œil (+6,4 % mesurés à 54 px) et se détache du plan des autres. Il vient APRÈS les déplacements et AVANT les rotations.
- ⚠️ **`.card3d:active` doit s'écrire `:not(.is-dragging)`** : même spécificité que `.card3d.is-dragging` (une pseudo-classe compte comme une classe), donc la dernière règle du fichier gagne. Le pointeur restant enfoncé tout le glisser, son `--card-pop: .96` mangeait les deux tiers de l'agrandissement.
- ⚠️ **`draggable={false}` sur l'illustration, et `-webkit-user-drag: none` sur `.unit-art`** : sans ça, le premier pixel d'un glisser démarre le **glisser natif d'image** du navigateur, qui émet aussitôt un `pointercancel` — le geste du jeu meurt avant d'exister, et rien dans le code ne le dit.
- ⚠️ **`touch-action: none` sur les seules cartes `[data-draggable]`** : `manipulation` laisse au navigateur le droit de réquisitionner le geste pour un défilement, avec le même `pointercancel` à la clé. Et pas sur `.card3d` en général — la même carte servira une grille qui DÉFILE (DeckBuilder, boutique), où confisquer le geste empêcherait de faire défiler la liste.
- ⚠️ **La capture du pointeur est indispensable au glisser** : une souris n'a pas la capture implicite du tactile, donc sans elle le geste s'arrête dès que le curseur quitte la carte. Corollaire : `pointerleave` ne doit **pas** annuler pendant un glisser (il reste émis au franchissement de la boîte).
- ⚠️ **`transition: none` sur la carte glissée** : avec la transition du repos, elle rattraperait le doigt avec 140 ms de retard.
...
- **Icônes et couleurs de monnaie sont définies une seule fois** — `components/ui/currency.ts`, avec la primitive **`<Amount currency value sign />`** comme point de rendu. ⚠️ La table vivait en privé dans `ProgressionStats`, donc illisible de l'extérieur, et les glyphes étaient réécrits à la main dans une vingtaine d'endroits : les gemmes empruntant une couleur de **tier** qui valait alors exactement `--color-gold`, gold et gemmes sortaient dans la même couleur sur le seul écran qui montre les deux côte à côte. ⚠️ Les gemmes ont depuis leur propre teinte (`--color-violet`) : une monnaie qui emprunte la couleur d'autre chose se fait repeindre le jour où cette autre chose change d'avis, et c'est arrivé. Le module porte aussi `fmt` (`Intl.NumberFormat('fr-FR')`) et **`CURRENCY_BY_WIRE`** (pont entre la clé du joueur `gold` et celle du fil `golds`). 💰 et non 🪙, qui retombe en disque gris.
- ⚠️ **Il n'y a qu'UNE palette de tier, et elle vit dans `three/cardPalette.ts`.** `--color-tier-1..5` (`styles/index.css`) en est le **miroir**, à l'identique — Tailwind ne peut pas lire un module TS, `three/` ne peut pas lire une feuille, donc `test/card-palette.test.ts` est le **seul** filet contre leur dérive (même situation que `tiers.js` / `logic/Tiers.ts`). Les deux palettes ont vécu **décalées d'un rang** : un tier 3 sortait violet sur le plateau et bleu en main, sur la vignette de boutique et dans le DeckBuilder.
- ⚠️ **Une classe `tier-N` dit un TIER, jamais une teinte.** `text-tier-4` servait de violet générique sur sept écrans et `text-tier-5` d'or sur deux — gemmes, rareté Légendaire, difficulté Vétéran, chips d'attribut, épingle de boutique, mission Engagée. Toutes se seraient fait repeindre au premier changement de palette. Les trois teintes empruntées portent maintenant leur propre nom (`--color-steel`, `--color-azure`, `--color-violet`), nommées par la **teinte** et non par un usage, justement parce qu'elles n'en ont pas un seul. ⚠️ `--color-azure` vaut la même chose que `--color-player` : deux décisions indépendantes qui coïncident, pas une duplication à replier.
...
- ⚠️ **Une pastille de notification se pose EN SURIMPRESSION, jamais dans le flux** (`AppFooter.DockTile`, pastille de palier de `ProgressionPills`) : l'en-tête en portrait n'a plus une seule ligne de large (390 px d'écran pour 355 de contenu), une pastille qui coûte sa largeur y fait passer les soldes à la ligne. Corollaire : elle se montre à **toutes** les tailles. C'est le décompte d'XP, lui, qui se cache sous `sm`.
- ⚠️ **L'en-tête montre le PSEUDO en portrait**, resserré (`max-w-[4rem]`) et jamais escamoté : un avatar est une image choisie, pas un nom. Quand la ligne déborde c'est le pseudo qui cède (`min-w-0` sur `ProfilePill`, `shrink-0` sur `ProgressionPills`) — un nom tronqué se lit encore, un solde tronqué serait un chiffre faux.
...
⚠️ **Le même raccourcissement touche tout `min-h-dvh` / `h-dvh`**, et une rotation aller-retour le fait disparaître (signature du bug). La racine des écrans non immersifs **et** celle de `GameScreen` / `GameScreenPvp` lisent **`--app-h`** (`h-[var(--app-h,100dvh)]`), posée par `app/viewportHeight.ts` à la hauteur de l'**écran** — seulement en standalone et seulement quand le viewport est plus court. Tout nouvel écran plein cadre doit la lire aussi. **Non traité** : les couches `fixed inset-0` (modales, overlays de partie) et les bancs de dev.
...
- ⚠️ **L'encart est PLUS GRAND en mobile qu'en desktop** (156 px contre 120), ce qui a l'air d'un contresens et n'en est pas : le survol n'existant pas au doigt, l'overlay y est posé en permanence et doit contenir ses commandes en entier — or `.btn` y porte la cible tactile de 44 px, soit 148 px pour trois boutons. Dans les 90 px d'avant, le troisième n'était pas décalé : l'encart **rogne** (`overflow: hidden`, exigé par le rayon de l'image), il était donc invisible et intapable. Rétrécir les boutons aurait cassé la règle des 44 px ; c'est la boîte qui cède. L'overlay porte en plus un `overflow-y: auto` — le filet qui garantit qu'un quatrième bouton resterait **atteignable** au lieu d'être retiré de l'écran sans un mot.
