...
⚠️ **La barrière de lancement de combat est indexée par ROUND** (`MatchRelay` : `ackRound`, `combatStartAcks`, `terrainByRound`), et ce n'est pas de la précaution. Les deux joueurs ne traversent pas la fin d'un round à la même vitesse — récapitulatif 22 s, Phase Shopping 45 s, préparation 60 s : l'un peut avoir tapé PRÊT pour le round suivant quand l'autre quitte à peine le précédent. `round:next_ready` vidait alors la barrière **sans regarder de quel round il parlait**, effaçant l'acquittement déjà posé ; chaque client n'acquittant qu'une fois par round, elle ne repassait **jamais** à deux et les deux joueurs restaient sur « En attente de l'adversaire… » indéfiniment — le match figé, sans erreur ni déconnexion. Constaté sur le match `d388310d` au round 4. `round:next_ready` n'est donc plus qu'un relais, et la barrière se referme sur elle-même. Verrouillé par `pvp-relay.test.ts`.

⚠️ **Une barrière à moitié franchie ne dure pas** : `BARRIER_TIMEOUT_MS` (180 s, armé au premier acquittement) la tranche, selon la règle de Marvel Snap — on joue le round si on PEUT le jouer, sinon le silencieux perd (`endMatch(..., 'timeout')`, le même forfait qu'une déconnexion dont la grâce a expiré). C'est le second gel possible du mode, distinct du précédent : un client suspendu par l'OS ou un onglet gelé n'acquitte jamais **sans se déconnecter pour autant**, et seule la déconnexion franche était traitée.

- ⚠️ **« Jouer sans lui » n'est possible que si son BOARD est arrivé.** Le client présent attend aussi `round:board_ready` (`waitForOpponentBoard`) et le serveur n'en garde **aucune copie** — un `round:go` sans board déplacerait le gel au lieu de le lever. La grâce ne vaut donc que dans le cas où seul l'acquittement manque ; c'est structurellement rare, board et acquittement partant du même appel synchrone.
- ⚠️ **« Une seule grâce » est une CONSÉQUENCE, pas un compteur** : après un `round:go` de grâce, un client toujours muet n'annoncera pas non plus le board du round suivant — la barrière suivante retombe donc sur le forfait, sans qu'aucun état ne soit à tenir.
- ⚠️ **Le délai ne borne pas la lenteur d'un joueur, il détecte un client MORT**, d'où sa générosité. Le pire écart légitime entre les deux acquittements se dérive des chronos du client, qui font tous avancer une partie sans le moindre geste : écart d'animation ×1/×4 sur un long combat (~27 s) + récapitulatif (22 s) + Shopping (45 s) + préparation (60 s) ≈ **155 s**. En dessous, on couperait la partie d'un joueur qui joue.
...
### ✅ Ce que les logs ont prouvé — huit causes, sur quatre duels

Le premier duel enregistré de bout en bout (match `779040f9`, 5 rounds) a
tranché le suspect n°1 et fait apparaître **deux causes que personne n'avait
nommées**. Le duel suivant (`a9c8c4a6`), une fois ces trois-là corrigées, a
confirmé les **trois suspects restants**. Un troisième (`3ebfa22f`) a montré une
sixième cause, d'une famille encore différente — non plus ce que le plateau
contient ni l'ordre où il s'énumère, mais ce qui **survit d'un round à l'autre**.
Un quatrième (`7ce04deb`) en a montré deux de plus, dont une que le log ne
pouvait PAS voir — elle a lieu après le dernier tick. Les huit sont corrigées ;
cf. « Duel en ligne — les huit causes de divergence » plus bas, et
`client/src/test/pvp-determinism.test.ts`.

⚠️ **C'est l'ordre de découverte qui compte** : le diff s'arrête à la PREMIÈRE
différence, donc chaque cause en masque d'autres. Un rapport « diverged » n'est
jamais la liste des pannes, c'est la plus précoce.

| Duel | Round | Ce qui divergeait | Nature |
|---|---|---|---|
| `779040f9` | 5 | `blocked_cells` du terrain | **suspect n°1, confirmé** |
| `779040f9` | 2, 3, 4 | `attack_timer` / `move_timer` d'un survivant | cause **non anticipée** |
| `779040f9` | 1 | `combat_end.winner` | **fausse** divergence — l'outil lui-même |
| `a9c8c4a6` | 2 | la case où une unité se déplace | **suspects n°2 et n°4** |
| `a9c8c4a6` | 3, 4 | l'ordre des `stat_change` d'attribut | **suspects n°2 et n°3** |
| `3ebfa22f` | 5 | le `max_hp` d'une unité morte au round d'avant | cause **non anticipée** |
| `7ce04deb` | 5 | la case d'arrivée d'une téléportation | **suspect n°4**, deuxième copie |
| `7ce04deb` | 2 à 5 | les survivants et les PV — **hors du log** | cause **non anticipée** |
...
**Les trois suspects restants ne se sont pas manifestés sur CE duel** — ils ont
tous les trois été confirmés par le **suivant** (`a9c8c4a6`), qui n'a pu les
faire apparaître qu'une fois les trois premières causes retirées :

2. **L'ordre des tableaux d'unités est inversé entre les deux clients.**
   `Board.getUnitsOnSide` balayait col-major, row **croissante** : un camp aux
   ...
   `findAttackTarget` (`d < bestDist`, premier arrivé), le tri stable par
   distance de Chebyshev du déplacement, le `reduce` de provocation,
...
3. **Le tri d'initiative n'a pas de départage de camp** : `[...playerUnits,
   ...
   jouer la même carte), le tri stable tranche dans deux sens opposés. **Le même
   tableau sert au balayage des morts**, dont l'ordre décide des `stat_modifier`
   d'attribut (ils ne comptent que les unités encore vivantes).
...
⚠️ **La leçon de méthode : une divergence en cache une autre.** Le premier duel
ne pouvait pas montrer ces trois-là — le diff s'arrête à la PREMIÈRE différence,
et le terrain comme les horloges divergeaient dès le tick 0. Il a fallu deux
duels et deux lots de corrections pour vider la pile. C'est aussi pourquoi le
filet de `pvp-determinism.test.ts` ne teste pas une cause mais l'**invariant** :
un même combat physique, simulé dans les deux repères, doit rendre le même log.
...
## ⚠️ Duel en ligne — les huit causes de divergence, et ce qui les ferme

Elles ont **la même forme, et une seule** : quelque chose qui n'a pas le même
sens dans les deux repères. Le monde du rôle B est le reflet de celui de A, le
combat est simulé localement des deux côtés sans qu'aucun résultat ne s'échange,
et le moindre écart fait diverger la partie entière — un désaccord final privant
**les deux** joueurs de leur gain (`result_mismatch`).

Deux familles, découvertes par deux duels successifs :

| | Ce qui n'était pas commun | Trouvé par |
|---|---|---|
| **Ce que le plateau CONTIENT** | les cases bloquées ; les horloges d'attaque et de déplacement ; le pouvoir donné par magie | duel `779040f9` |
| **L'ORDRE dans lequel il s'énumère** | le balayage des unités ; les voisines d'une case ; le camp qui agit en premier | duel `a9c8c4a6` |
| **Ce qui SURVIT d'un round à l'autre** | les bonus d'attribut d'une unité tombée au combat | duel `3ebfa22f` |
| **Ce qui arrive APRÈS le dernier tick** | la réanimation d'attribut, donc les survivants et les dégâts | duel `7ce04deb` |
...
jamais vrai — c'est toujours le camp du propriétaire qui a tort.

⚠️ **La seconde famille ne pouvait pas apparaître avant que la première soit
corrigée** : le diff s'arrête à la première différence, et le terrain divergeait
dès le tick 0. Corollaire de méthode — une divergence en cache une autre, et
c'est le **filet** (l'invariant, pas la cause) qui finit le travail.
...
applique `mirrorCells(blocked_cells)` quand `deps.mirroredRole` est levé, ce
...
### 4. Le plateau ne s'énumérait pas dans le même ordre (`Board.mirroredFrame`)

`Board.getUnitsOnSide` balayait les rangées en ordre **croissant**. Le même camp
étant en rows 0–3 chez son propriétaire et en 7–10 chez son adversaire, les deux
clients obtenaient l'ordre relatif **inverse** — et cet ordre est de la logique
de jeu : il survit tel quel dans `CombatManager.playerUnits` / `enemyUnits` pour
toute la durée du combat et départage les égalités.

Même problème pour `getNeighbors` : `row - 1` et `row + 1` désignent deux
directions **physiques opposées** d'un client à l'autre, et le BFS de
`findPath` rend le *premier* plus court chemin trouvé (`stepTowardOrNearest`, la
première voisine la plus proche).

Le plateau porte donc un drapeau **`mirroredFrame`**, posé une fois par
`GameSession` : le balayage des rangées et l'énumération des voisines se font
dans le repère de **référence** (celui du rôle A).

- Constaté sur `a9c8c4a6`, round 2, tick 85 : quatre cibles à **égale distance
  de Chebyshev**, le tri de déplacement étant stable. Un client a trouvé sa
  première candidate déjà à portée et n'a pas bougé ; l'autre en a trouvé une
  différente et a avancé d'une case.
- Rounds 3 et 4 : le même ordre décidait de la suite des `stat_change` émis par
  `AttributeManager._triggerStatModifiers`.
- ⚠️ **Seul le rôle B change.** Le rôle A balaie exactement comme avant, donc
  solo, arcade, tournoi, tutoriel et simulation d'équilibrage sont **inchangés
  au bit près** — aucun golden n'a bougé. C'est la propriété qui rend ces
  corrections sûres : elles n'introduisent pas un nouvel ordre, elles imposent
  aux deux clients **celui qui existait déjà** d'un des deux côtés.

### 5. Le camp qui agit en premier

`[...playerUnits, ...enemyUnits]` met « mes » unités en tête sur **chaque**
client. Ce tableau sert à deux choses, et les deux en dépendent :

- le **balayage des morts** de la phase 5 — or chaque mort déclenche les
  `stat_modifier` d'attribut, qui ne comptent que les unités **encore
  vivantes** : deux unités qui tombent au même tick, une de chaque camp, ne
  donnent pas les mêmes bonus selon l'ordre ;
- le **tri d'initiative**, dont c'est le dernier recours à égalité parfaite
  (même initiative, même vitesse, même `card_id` — ce qui arrive dès que les
  deux joueurs jouent la même carte).

`CombatManager._frameOrderedUnits()` rend les deux camps dans l'ordre du repère
de référence. ⚠️ Le départage `_frameSide` ajouté au comparateur est
**redondant** avec lui tant que `sort` est stable (garanti depuis ES2019) — le
retirer seul ne fait rien tomber ; il est gardé parce qu'il rend le tri
**autosuffisant** au lieu de dépendre de l'ordre du tableau qu'on lui passe.

⚠️ **L'enregistreur RECOPIE ce tri** (`CombatRecorder.initiativeOrder`) et avait
déjà dérivé : le départage par camp lui manquait, et l'outil rapportait `order`
divergent sur un duel que le moteur jouait pourtant à l'identique. Même famille
que la traduction du vainqueur — un outil de diagnostic qui crie au loup sur les
cas sains est pire qu'un outil absent.

### 6. Les bonus d'attribut d'une unité morte

`finishCombat` ne remettait à zéro que les unités **encore vivantes**. Une unité
tombée au combat gardait donc indéfiniment les bonus d'attribut de son dernier
round — `max_hp` gonflé compris — tant qu'elle restait au cimetière, et le round
suivant `applyStartOfCombat` ajoutait les nouveaux **par-dessus** les anciens.

Le seul chemin qui ramène une telle unité sur le plateau est la magie `revive` :
elle la rend à un pourcentage de ce `max_hp` périmé, et l'unité rejoue le round
suivant avec un palier de synergie que son camp n'atteint plus.

- Constaté sur `3ebfa22f`, round 5 : une seule unité en écart, de 20 PV.
  ⚠️ **C'est le log lui-même qui a fourni l'étalon** — l'attribut Guerrier donne
  +20 PV à partir de trois porteurs, et les rounds 1 à 4 le calibrent
  exactement (2 porteurs → +10, 3 → +20) ; le round 5 n'en comptait plus qu'un.
  Le bonus ne pouvait donc être que périmé, et c'est le **propriétaire** qui
  avait tort.
- `finishCombat` balaie désormais **tous les participants**, neutralisés
  compris (`combatants`, capturé avant les filtres).
- C'est aussi un vrai bug de jeu, indépendamment du duel : une unité qui meurt
  et revient gardait la synergie du round précédent, et le cumul recommençait à
  chaque résurrection.
- Même geste que la remise à zéro des horloges (cause n°2), et même règle : un
  état qui n'a pas de raison de survivre au combat se **supprime**.

### 7. Le SECOND énumérateur de voisines — la téléportation

`CombatManager._teleportPlan` a sa **propre** liste de cases adjacentes, avec sa
propre priorité (rangées avant colonnes), et retient la **première** libre :
`row - 1` puis `row + 1`. La correction faite à `Board.getNeighbors` (cause n°4)
ne l'atteignait donc pas. Constaté sur `7ce04deb`, round 5, tick 64 : la même
téléportation atterrit en `(3,1)` chez un client et en `(3,3)` chez l'autre.

Les deux énumérateurs passent désormais par **`Board.rowNeighbourOffsets()`**, et
le balayage de repli de `_teleportPlan` par **`Board.rowScan()`**.

⚠️ **La leçon : une règle de repère recopiée à deux endroits est une règle qu'on
corrige à un seul.** C'est la deuxième fois — l'enregistreur recopiait déjà le
tri d'initiative (cause n°5). Avant d'ajouter un énumérateur de cases, se
demander si `Board` n'en a pas déjà un.

### 8. Ce qui arrive APRÈS le dernier tick — la réanimation d'attribut

`AttributeManager.applyEndOfCombat` ne regardait que `this.playerUnits`, pour
**tous** ses effets. Or l'un d'eux n'est pas une ressource du joueur : `revive`
remet une unité sur le plateau. En duel, l'unité réanimée d'un joueur
ressuscitait donc chez **lui** et restait morte chez son **adversaire** — et
comme `finishCombat` recense les survivants *après* la réanimation, les deux
clients ne comptaient pas les mêmes survivants, donc pas les mêmes dégâts de fin
de combat.

Constaté à l'écran sur `7ce04deb` : un joueur voyait **quatre** survivants (dont
« Fracas le guerrier magique », 16 ATK), son adversaire **trois** — et 828 PV là
où l'autre en affichait 860, soit exactement 16 × 2.

`applyEndOfCombat` traite désormais les **deux camps** (`_applyEndForSide`) :
`revive` vaut des deux côtés, les effets de **ressource** (pioches, emplacement,
multiplicateur, Shopping) restent au joueur, où ils sont les seuls à avoir un
destinataire. `finishCombat` replace le réanimé adverse sur une case ennemie,
pendant exact de la boucle joueur.

- ⚠️ **C'était invisible dans le log**, et c'est le vrai enseignement : le
  combat n'est pas tout le round. 162 ticks rigoureusement identiques des deux
  côtés, et un verdict `ok` — alors que les deux joueurs voyaient deux scores
  différents. D'où l'**épilogue** ci-dessous.
- Conséquence de jeu, indépendamment du duel : en solo l'IA ne bénéficiait
  jamais de son propre `revive`. C'était le seul `timing` d'attribut réservé au
  joueur, tous les autres s'appliquant déjà aux deux camps.

### L'épilogue du log — le round ne s'arrête pas au dernier tick

`CombatRecorder.epilogue(session, result)` enregistre ce que `finishCombat`
décide : vainqueur canonique, **survivants retenus** (`owner:card_id` + ATK,
réanimés compris), somme d'ATK par rôle, et les **PV des deux joueurs**.
`pvplog.diff` le compare en dernier, sous le `kind` **`epilogue`**.

- ⚠️ **`GameController._onCombatFinished` appelle `finishCombat()` AVANT
  d'expédier le log**, et l'ordre n'est pas un détail : il partait auparavant
  *avant* la résolution, si bien que cette moitié du round n'était jamais
  enregistrée.
- Les PV sont un contrôle légitime : chaque client est autoritaire sur les
  siens et reçoit ceux de l'autre au round suivant, donc les deux calculent la
  **même paire** tant que rien ne diverge. C'est très exactement ce que les deux
  joueurs lisaient à l'écran.
- Un log antérieur au champ (`epilogue` absent d'un côté) ne conclut rien —
  `diffEpilogue` rend `null` plutôt que d'inventer une divergence.

### Tests

`client/src/test/pvp-determinism.test.ts` (26 golden tests) et les deux cas
ajoutés à `pvp.test.ts`.

**Le cœur du fichier n'est pas un test par cause, c'est le FILET** : deux
`GameSession` complètes, l'une dans le repère de référence et l'autre dans son
miroir, jouent le **même combat physique** et doivent rendre le même log
canonique — celui-là même que `/api/admin/pvp-logs` compare. 300 combats semés
(positions, stats et pouvoirs variés, sur un terrain aux cases symétriques pour
que seul l'ORDRE puisse encore diverger), plus un cas d'égalité parfaite en
goulet. **Ce filet attrape six des huit causes à la fois**, y compris celles que
personne n'a encore nommées — c'est ce qui le distingue des tests par cause, qui
ne défendent que ce qu'on savait déjà.

⚠️ Il fait passer les cases bloquées par une **vraie `GameSession`** de chaque
rôle, jamais par un miroir écrit à la main : sans ça il ré-implémenterait la
correction au lieu de l'exercer, et resterait vert avec le miroir retiré
(constaté). Même raison pour l'enregistreur : c'est le vrai `CombatRecorder` et
le vrai `pvplog.diff` qui rendent le verdict.

⚠️ **Un combat isolé ne suffit pas** : la cause n°6 est une rémanence d'un round
sur l'autre, invisible d'une seule `startCombat`. Le filet a donc une seconde
forme, **multi-rounds** : trois rounds enchaînés, avec le VRAI aller-retour
réseau (`sendOwnBoard` → `reconstructOpponentUnits`, le module réel, sa mock de
`PvpConnection` près) et une résurrection après chaque combat — le seul geste
qui ramène sur le plateau une unité sortie neutralisée. Sans cette
résurrection le filet reste vert avec la correction retirée (constaté) : les
morts resteraient morts, et leur `_stat_bonuses` avec eux.

⚠️ Les 300 graines ne peuvent PAS produire l'égalité parfaite — elles nomment
les cartes par leur camp (`A_0`, `B_0`), donc `card_id` tranche toujours. D'où
le cas du **goulet**, où les deux camps jouent la même carte et se disputent la
seule case libre de la rangée centrale. ⚠️ Et il faut une **course** pour que
l'ordre d'action se voie : deux jumelles qui s'entretuent meurent de toute
façon, la phase d'attaque résolvant tous les coups avant le décompte des morts.

Tout est **éprouvé dans les deux sens** — miroir du terrain retiré, remise à
zéro des horloges retirée ou déplacée dans `resetCombatStats`, traduction du
vainqueur rendue identité, `power_*` retirés du payload, balayage des rangées
toujours croissant, ordre des voisines figé, ordre de camp retiré du moteur,
départage manquant dans la copie de l'enregistreur, remise à zéro finale
restreinte aux unités vivantes, liste d'adjacence de la téléportation figée, et
réanimation restreinte au camp local : chacune de ces régressions fait passer la
suite au rouge.

⚠️ **Un piège du harnais, trouvé en le corrigeant** : le champ de la carte est
`power.power_speed`, pas `power.speed`. Écrit `speed`, le pouvoir hérite du
`9999` d'`Unit` — « pas de pouvoir » — et **aucun** pouvoir ne part. Les 300
graines ont tourné un temps avec des pouvoirs muets, et ne prouvaient donc rien
à leur sujet : c'est la correction de ce champ qui leur a fait attraper la
cause n°7.
...
- Verrouillé par `client/src/test/magie-offer.test.ts` (39 golden tests sur le module pur) et `client/src/test/shopping.test.ts` (81, au niveau `GameSession`). Tous **éprouvés dans les deux sens** : filtre retiré, poids égalisés, tirage avec remise, `default: true`, défaut de `rarityOf` déplacé — chacune de ces cinq régressions fait passer la suite au rouge.
...
- ⚠️ **La jauge de pouvoir de la carte 3D est TOUJOURS dans le balisage**, masquée en `display:none` tant que l'unité n'a pas de pouvoir — c'est `updateUnitEl` qui la révèle. `_inner()` ne s'exécute qu'au **spawn** : émise conditionnellement, elle n'existait pas sur une unité née sans pouvoir, et `grant_power` lui en posait un que la carte ne pouvait plus montrer (`querySelector` sur un élément absent). Le tooltip, lui, est rendu par React depuis l'unité vivante — d'où une magie qui « marchait » partout sauf sur le board. Même patron que `unit-vet-badge` et `unit-medallion`, les deux autres éléments conditionnels du fichier : présence constante, visibilité pilotée par l'update.
...
- ⚠️ **C'est elle qui a ouvert le ciblage de la MAIN** (`duplicate_card` l'a rejointe depuis), et c'est ce qui a coûté le plus cher au HUD : `HandBar` se masquait sur `combatActive`, or la Phase Shopping a lieu **après** le combat, drapeau encore levé. Elle porte donc la même exception que `GraveyardTray` pour le ciblage `revive` (`awaitingTarget === 'hand'`), et une carte **injouable y reste tapable** — c'est même souvent celle qu'on veut envoyer au cimetière. Rien à rebrancher pour une seconde magie de main : le HUD ne connaît que `awaitingTarget`.
...
### Duplication (`duplicate_unit`, `duplicate_graveyard_unit`, `duplicate_card`)

Trois sources, **une seule destination : la main**. `duplicate_unit` copie la carte d'une **unité du terrain**, `duplicate_graveyard_unit` celle d'une **unité du cimetière**, `duplicate_card` copie une **carte de la main** — et c'est tout ce qui les distingue, d'où la même lecture de `value` (`MagieEffect.duplicateCopies`) et le même point de livraison (`GameSession._pushHandCopies`).

⚠️ **Les deux duplications d'unité sont LA MÊME méthode** (`_duplicateFromUnit`), et c'est le point qui commande le reste : ce qu'on lit sur une unité — sa `card_id`, puis la carte du catalogue — ne dépend pas de l'endroit où elle se trouve. Seule change la collection d'où vient la cible, donc la famille de ciblage. Les écrire deux fois aurait donné deux réponses à la même question.

- ⚠️ **`duplicate_unit` rend la CARTE DE CATALOGUE, pas l'unité.** Rien de ce que l'unité a acquis sur le terrain ne voyage : ni bonus de Shopping (`_shopping_bonus`), ni vétérance, ni PV courants, ni bouclier, ni pouvoir posé par `grant_power`. C'est ce qui sépare une duplication d'un clonage — sans cette étanchéité, la magie **rendrait deux fois un investissement de Shopping**, alors même que `_transferShoppingBonuses` s'échine à ne le transférer qu'une fois.
- ⚠️ **La RÈGLE DU DOUBLON ne pèse que sur la copie prise au BOARD**, et c'est ce qui donne aux deux provenances des tempos opposés — la seule différence de design entre elles :

| Source | La copie est… | Ce qu'on achète |
|---|---|---|
| **Terrain** (`duplicate_unit`) | injouable tant que l'original vit — sauf à le désigner comme **matériau** (sacrifice / fusion / héritage / transformation) ; grisée en main | un **remplaçant** mis de côté, qui prend sa valeur quand l'original tombe |
| **Cimetière** (`duplicate_graveyard_unit`) | jouable **tout de suite** — une unité neutralisée n'est pas un doublon vivant | une **seconde chance**, sans rien retirer du cimetière |

  C'est le sens même des deux magies, pas un effet de bord à rattraper.
- ⚠️ **`duplicate_graveyard_unit` LAISSE le corps au cimetière**, là où `revive` l'en sort pour le reposer sur le terrain : les deux partagent la famille de ciblage et rien d'autre. Le corps reste donc **matériau d'invocation** — la magie rend une carte *sans* dépenser la dépouille, et le corps disparaît au lancement du combat comme n'importe quel autre non consommé.
- ⚠️ **`duplicate_card` LAISSE sa cible en main**, là où `hand_to_graveyard` la retire — les deux partagent le geste (taper une carte) et rien d'autre. C'est la seule branche de `applyMagieOnHandCard` qui ne fait pas de `splice`, et elle rend `null` : aucune unité n'est créée.
- **La copie est prise TELLE QU'ELLE EST**, remises comprises (`_original_sacrifice`, `_removed_materials`, `_free_transformation`) : le joueur duplique la carte qu'il a sous les yeux, avec le coût que son tooltip annonce. Elle rejoint donc le même groupe que l'originale dans la main (badge ×N) — `GameController._groupHand` clé sur l'id **et** le coût.
- ⚠️ **`value` compte les copies, et une Valeur à 0 en rend UNE** — le repli de `powerValue` (`||`, jamais `??`), et pour la même raison : 0 est le **défaut du champ** de l'admin. Lu strictement, il ferait d'une magie offerte un blanc qui aurait encaissé son contrecoup au passage. Le contrecoup, lui, est prélevé **une seule fois** quel que soit le nombre de copies.
- ⚠️ **Ni `boardUnitCount` ni `graveyardCount` ne suffisent** : `_cataloguedUnits()` écarte les unités dont la carte a quitté le catalogue, et `MagieOffer` porte les deux compteurs dédiés `duplicableUnitCount` / `duplicableGraveyardCount`. Même famille que `_defusableFusions` / `_poweredUnits`, et pour la même raison — la règle sert le **ciblage** et la **pertinence de l'offre**, elle ne doit exister qu'à un endroit. `revive`, lui, continue de se contenter de `graveyardCount` : il ne lit que l'unité, jamais sa carte.
- ⚠️ **La carte est résolue AVANT le contrecoup** (`_duplicateFromUnit`) : un coût en PV prélevé pour une copie qui n'arrive jamais serait pire qu'un refus. Le filtre d'offre rend le cas inatteignable ; cette garde est ce qui l'en empêche pour de bon.
...
⚠️ **Rejouer le même `id` ne suffit PAS à jouer le même plateau** : `blocked_cells` est une donnée **positionnelle**, et le monde du rôle B est le reflet de celui de A. Le rôle B applique donc les cases **miroitées** (`GameSession` + `logic/BoardMirror`, cf. « Duel en ligne — les huit causes de divergence ») — sans quoi une case décrite en rangée 4 est voisine du camp de A chez A et du camp de B chez B, et ligne de vue comme contournement BFS divergent dès les premiers ticks. Constaté sur un vrai duel (`BOARD_013`, première conséquence au tick 10). Corollaire : `Board.blockedCells()` est la seule lecture juste pour le rendu et pour le log — pas `boardData.blocked_cells`.
