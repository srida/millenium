...
- ⚠️ **PAS de badge de tier, et les deux badges sont à GAUCHE.** Le tier EST la couleur du cadre (`three/cardPalette`), ce que la vignette 2D ne savait pas faire : un « T3 » par-dessus un cadre violet dit deux fois la même chose et mange le seul coin qu'un éventail laisse voir. Le coin libéré prend le **coût** (`◈N`, haut-gauche), le compte d'exemplaires descend en **bas-gauche** — c'est la lisière GAUCHE qui reste visible sous la carte suivante, à droite le `×N` disparaissait. ⚠️ **La carte du PLATEAU suit les deux coins** : `unit-mat-badge` (ce qu'elle vaut) en haut, `unit-vet-badge` (ce qu'elle cumule) en bas. Le nom cède sa marge (`.card3d.has-count`) plutôt que de passer dessous.
