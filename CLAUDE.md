...
### Duplication (`duplicate_unit`, `duplicate_card`)

Deux sources, **une seule destination : la main**. `duplicate_unit` copie la carte d'une **unité du terrain**, `duplicate_card` copie une **carte de la main** — et c'est tout ce qui les distingue, d'où la même lecture de `value` (`MagieEffect.duplicateCopies`) et le même point de livraison (`GameSession._pushHandCopies`).
...
- ⚠️ **Conséquence assumée de la RÈGLE DU DOUBLON** : tant que l'original vit, la copie n'est invocable qu'en désignant ce doublon comme **matériau** (sacrifice / fusion / héritage / transformation) — une invocation normale la refuse, et la main l'affiche grisée. Elle prend sa valeur quand l'original tombe : on met un **remplaçant** de côté, on ne pose pas un second corps. C'est le sens même de la magie, pas un effet de bord à rattraper.
...
- ⚠️ **`duplicate_unit` ne se contente pas de `boardUnitCount`** : `GameSession._duplicableUnits()` écarte les unités dont la carte a quitté le catalogue, et `MagieOffer` porte le compteur dédié `duplicableUnitCount`. Troisième membre de la famille de `_defusableFusions` / `_poweredUnits`, et pour la même raison — la règle sert le **ciblage** et la **pertinence de l'offre**, elle ne doit exister qu'à un endroit.
...
- Verrouillé par 15 golden tests dans `shopping.test.ts` et 3 dans `magie-offer.test.ts`, tous **éprouvés dans les deux sens** : `duplicateCopies` passée en `??`, copie par référence au lieu d'un objet neuf, `duplicate_card` retirant sa cible, ciblage non filtré par le catalogue, pertinence rabattue sur `boardUnitCount`, les deux types oubliés dans `isMagieRelevant` (le `default: false`), et la copie emportant les acquis de l'unité — chacune de ces sept régressions fait passer la suite au rouge.
