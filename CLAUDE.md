...
- **L'échelle (`value_per`) vit dans `logic/EffectScale`**, seul lecteur, partagé par les attributs et les terrains. `active_unit` (mes alliés vivants) · `enemy_unit` · `<ARCH_id>` (les **ennemis** qui le portent — forme historique) · `enemy:<ARCH_id>` (la même, explicite) · `ally:<ARCH_id>` · `one` (×1). ⚠️ Elle multiplie un **montant**, jamais un facteur : `stat_modifier` ne la lit pas.
- ⚠️ **Le bouclier d'ATTRIBUT a un `scale_default`** (× alliés vivants) que les autres types n'ont pas : `ARCH_066` tire 150 d'un `shield: 50` nu. Le défaut est **déclaré au registre et lu par le moteur** ; `one` est le seul moyen d'y renoncer.
- ⚠️ **Sur un TERRAIN, l'échelle se compte dans le camp de chaque unité TOUCHÉE** — un effet de terrain frappe les deux camps, il n'a pas de camp à lui.
- **La cible d'un `revive` d'attribut** : `first` (défaut) · `highest_hp` · `highest_atk`, départage par `card_id`. ⚠️ `neutralized_ally` est le synonyme historique de `first`. Une cible inconnue **retombe sur `first`**, jamais sur « rien ».
- ⚠️ **La PORTÉE n'est pas un paramètre** : une magie s'applique **une fois** (Shopping), un attribut et un terrain **à chaque combat**. Un bonus permanent y cumulerait (+5 ATK → +25 au 5ᵉ round). Les deux gestes vivent sur `Unit` — `applyStatBonus` (combat) et `applyPermanentStat` / `applyPermanentModifier` (permanent, tracé dans `_shopping_bonus`) ; c'est le **type** qui choisit, pas la donnée.
...
- **`GameState.gainPlayerHp(n)` est le SEUL crédit de PV joueur** — `player_hp_bonus`, `sacrifice_card_hp` et `drain_life` y passent tous. Il plafonne à `PLAYER_HP_CAP` et ne débite jamais (un montant négatif rend 0). ⚠️ Le geste était écrit trois fois, dont une avec un plafond **`1000` littéral**.
- **`GameSession._sendToGraveyard(unit)`** neutralise et empile — trois appelants (`destroy_unit`, `hand_to_graveyard`, débordement de `defuse_fusion`). ⚠️ Il ne fait **pas** `removeUnit` : une carte de la main n'occupe aucune case.
- ⚠️ **Les onze magies « avancées » ne se replient PAS sur une primitive de transfert générique**, et c'est un constat, pas un manque de temps : leur substance est le **résolveur** (sept distincts pour huit effets — carte de catalogue, carte avec ses remises, carte du deck au tier voisin, matériel à double repli, l'unité, ses matériels nommés, rien). Une destination générique `board` court-circuiterait en plus `InvocationManager`, par où **tout** ce qui se pose doit passer.
