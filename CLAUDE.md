...
**Niveaux d'accès** : l'écriture sur `cards` / `attributes` / `powers` passe par le middleware d'auth générique ; `boards`, `magies`, `missions` et `decks` exigent en plus `requireSiteAdmin` (`auth.js`).
