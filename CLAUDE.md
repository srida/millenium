...
⚠️ **Le même raccourcissement touche tout `min-h-dvh` / `h-dvh`**, et une rotation aller-retour le fait disparaître (signature du bug). La racine des écrans non immersifs lit **`--app-h`** (`h-[var(--app-h,100dvh)]`), posée par `app/viewportHeight.ts` à la hauteur de l'**écran** — seulement en standalone et seulement quand le viewport est plus court. **Non traité ailleurs** : en jeu, `PhaseControls` (`absolute bottom-0`) flotte encore 62 pt au-dessus du bas réel ; les écrans immersifs gardent leurs `dvh`.
