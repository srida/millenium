...
⚠️ Limite connue : **rien ne débloque une barrière à moitié franchie**. Un `combat_start_ack` perdu, ou un client qui n'acquitte jamais sans se déconnecter pour autant, laisse l'autre en attente — `PvpController.startCombat` sort tout de suite quand `_handshaking` est levé, y compris à l'expiration du chrono de préparation. La déconnexion franche, elle, est traitée (`handleDisconnect` donne la victoire à celui qui reste).
