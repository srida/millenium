...
//
// Invité : repli sur les cartes de DÉPART (CORE_*), exactement la dotation d'un
// compte neuf. Le jeu se joue sans compte (auth optionnelle) : un invité sans
// aucune carte ne pourrait plus construire de deck, et ce qu'il bâtit reste
// valable s'il s'inscrit ensuite.
import { create } from 'zustand';
import * as AuthClient from '../data/AuthClient.js';
import * as CardDatabase from '../data/CardDatabase.js';

// Doit rester aligné sur STARTER_PREFIX de progression.js (serveur).
const STARTER_PREFIX = 'CORE';

function starterCardIds(): string[] {
  return ((CardDatabase as any).getAllCards() as { id: string }[])
    .map(c => c.id)
    .filter(id => String(id).toUpperCase().startsWith(STARTER_PREFIX));
}
