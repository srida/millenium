...
export function magieBehaviour(): Record<string, unknown> {
  ...
  for (const magie of load('magies.json') as any[]) {
    ...
    const gameState: any = {
      player_hp: 500, player_extra_draws: 0, player_draw_sources: [],
      player_guaranteed_draws: [], player_hand_modifiers: [],
      player_damage_multiplier_bonus: 0, _slotBonus: 0,
      grantLimitedBoardSlotBonus(n: number) { this._slotBonus += n; return n; },
    };
    ...
    out[magie.id] = {
      ...
      state: {
        ...
        modifiers: gameState.player_hand_modifiers, slot: gameState._slotBonus,
