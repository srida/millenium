...
// ⚠️ RETIRÉ : la technique du switch invisible (`<input type="checkbox"
// switch>`, https://github.com/tijnjh/ios-haptics) pour un vrai déclic
// Taptic sur iOS. Trois tentatives de correction (position dans le DOM,
// z-index, timing de l'action) n'ont réglé qu'une partie des symptômes —
// la dernière en date a fini par rendre TOUS les boutons inertes sur iOS
// (le tap déclenchait le switch mais plus l'action du bouton). `switch` est
// un contrôle natif tout neuf (Safari 17.4, 2024) qui imite un interrupteur
// physique, glissement compris : sa gestion tactile interne entre très
// probablement en conflit avec `usePressSquash`, d'une façon impossible à
// diagnostiquer sans Safari/iPhone réel — Chromium ne simule pas ce
// contrôle, ce qui a laissé passer la régression en test piloté. Retour
// visuel (relief + flash de couleur) et sonore (Web Audio) restent : ce
// sont eux qui doivent porter le retour sur iOS.
