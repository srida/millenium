...
function createRoundedTileGeo(THREE, size, radius) {
  const s = size / 2;
...
export class Board3D {
  ...
  _buildTiles() {
    ...
    const tileGeo = createRoundedTileGeo(THREE, tileSize, 0.082);
  ...
  _cameraFraming(combatMode) {
    ...
    const minRow = 0;
    var maxRow = PLAYER_ROWS - 3;
    var rowsVisible = PLAYER_ROWS + 1.5;
    if(showFullBoard) {
      maxRow = TOTAL_ROWS - 1;
    ...
    var heightForRows = (rowsVisible * CELL) / (2 * Math.tan(vFov / 2));
    var heightForCols = (COLS * 0.5 * CELL) / (2 * Math.tan(vFov / 2) * aspect);
    if(showFullBoard) {
  ...
  _updateTileColor(tile) {
    const { col, row, baseColor, baseEmissive = 0x000000, baseEmissiveIntensity = 0, isPlayer } = tile.userData;
    const k = `${col},${row}`;
    let color = baseColor;
    ...
    if (isPlayer) tile.material.opacity = opacity;
  }
